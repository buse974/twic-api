...
if [ ! $LINE = "[]" ]; then
	NAMEBASE=`uuidgen -t`
		...
		ffmpeg -y -i $P$NAME -f mjpeg -ss $DURATION25 -vframes 1 -pix_fmt yuvj420p -an $P$NAMEBASE-25.jpg	
		ffmpeg -y -i $P$NAME -f mjpeg -ss $DURATION50 -vframes 1 -pix_fmt yuvj420p -an $P$NAMEBASE-50.jpg	
		ffmpeg -y -i $P$NAME -f mjpeg -ss $DURATION75 -vframes 1 -pix_fmt yuvj420p -an $P$NAMEBASE-75.jpg	
        fi
fi
done
