...
CREATE TABLE IF NOT EXISTS `questionnaire` (
  ...
  `max_time` INT NOT NULL DEFAULT 2,
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `activity` (
  ...
  `object_value` INT NOT NULL DEFAULT 1,
  `object_data` VARCHAR(45) NULL,
  `target_id` VARCHAR(45) NULL,
  `target_name` VARCHAR(45) NULL,
  `target_data` VARCHAR(45) NULL,
  `date` DATETIME NULL,
  `user_id` INT NULL,
  PRIMARY KEY (`id`))
ENGINE = InnoDB
...
VIEW `research` AS
    ...
    FROM
        ...
        JOIN `program` `p`)
    WHERE
        (`c`.`program_id` = `p`.`id` AND `p`.`deleted_date` IS NULL) 
    UNION SELECT 
        `s`.`id` AS `id`,
        `s`.`name` AS `name`,
