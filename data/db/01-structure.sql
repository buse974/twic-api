...
-- jeu. 09 juil. 2015 11:00:28 CEST
...
CREATE TABLE IF NOT EXISTS `mail_doc` (
  ...
  `created_date` DATETIME NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_message_doc_1_idx` (`mail_id` ASC),
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `research` (`id` INT, `firstname` INT, `lastname` INT, `avatar` INT, `category` INT, `role` INT);
...
SELECT u.`id`, u.`firstname`, u.`lastname`, u.`avatar`, 'user' AS category, role.name as role
FROM `user` as u
INNER JOIN user_role as us ON us.user_id=u.id
INNER JOIN role ON us.role_id=role.id
WHERE us.role_id IN (4,5)
UNION 
SELECT c.`id`, c.`title`, c.`creator_id`, c.`picture`, 'course' AS category, null AS role
FROM `course` as c 
UNION
SELECT s.`id`, s.`name`, s.`short_name`, s.`logo`, 'school' AS category, null AS role
FROM `school` as s;
