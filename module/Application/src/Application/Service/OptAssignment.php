...
class OptAssignment extends AbstractService
{
    /**
     * @invokable
     * 
    ...
    {
        $m_opt_assignment = $this->getModel()
            ->setItemId($item_id)
            ->setMode($mode)
            ->setHasPg($has_pg)
            ->setPgNb($pg_nb)
    ...
    {
        $m_opt_assignment = $this->getModel()->setItemId($item_id);
        
        return $this->getMapper()->delete($m_opt_assignment);
    }
