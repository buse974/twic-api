...
class ItemUser extends AbstractService
{
  ...
  **/
  ...
  {
    return $this->getMapper()->select($this->getModel()->setItemId($item_id)->setId($id)->setSubmissionId($submission_id)->setUserId($user_id)->setGroupId($group_id));
  }
  ...
  */
  public function getOrCreate($user_id, $item_id, $submission_id = null)
  {
    ...
    if(null !== $submission_id && $m_item_user->getSubmissionId() !== $submission_id) {
      $this->getMapper()->update($this->getModel()->setId($m_item_user->getId())->setSubmissionId($submission_id));
    }
