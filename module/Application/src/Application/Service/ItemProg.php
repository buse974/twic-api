...
class ItemProg extends AbstractService
{
     ...
     * @param filter $filter            
     */
    public function getList($item = null, $start = null, $end = null, $course = null, $id = null, $filter = null)
    {
        $me = $this->getServiceUser()->getIdentity();
        $mapper = $this->getMapper();
        
        $res_item_progs = $mapper->usePaginator($filter)->getList($me, $item, $start, $end, $course, $id);
        foreach ($res_item_progs as $m_item_prog) {
            $m_item_prog->setUsers($this->getServiceUser()
                ->getListByItemProg($m_item_prog->getId()));
            if (in_array(ModelRole::ROLE_STUDENT_STR, $me['roles'])) {
                $m_item_prog->setItemProgUser($this->getServiceItemProgUser()
                    ->getListByItemProg($m_item_prog->getId(), $me['id'])
                    ->current());
            } else {
                $m_item_prog->setItemProgUser($this->getServiceItemProgUser()
                    ->getListByItemProg($m_item_prog->getId()));
        ...
        return ($filter !== null) ? ['count' => $mapper->count(),'list' => $res_item_progs] : $res_item_progs;
    }
    ...
    public function nbStart($school)
    {
        ...
        $ret['d'] = ['LC' => $this->getMapper()->nbStart($school, 1, true),'GW' => $this->getMapper()->nbStart($school, 1, false),'CP' => $this->getMapper()->nbSubmit($school, 1, true),'IA' => $this->getMapper()->nbSubmit($school, 1, false)];
        
        $ret['w'] = ['LC' => $this->getMapper()->nbStart($school, 7, true),'GW' => $this->getMapper()->nbStart($school, 7, false),'CP' => $this->getMapper()->nbSubmit($school, 7, true),'IA' => $this->getMapper()->nbSubmit($school, 7, false)];
        
        $ret['m'] = ['LC' => $this->getMapper()->nbStart($school, 30, true),'GW' => $this->getMapper()->nbStart($school, 30, false),'CP' => $this->getMapper()->nbSubmit($school, 30, true),'IA' => $this->getMapper()->nbSubmit($school, 30, false)];
        
        $ret['a'] = ['LC' => $this->getMapper()->nbStart($school, null, true),'GW' => $this->getMapper()->nbStart($school, null, false),'CP' => $this->getMapper()->nbSubmit($school, null, true),'IA' => $this->getMapper()->nbSubmit($school, null, false)];
    ...
    public function getListByUserAndCourse($course, $user)
    {
        ...
        foreach ($res_item_prog as $m_item_prog) {
            $res_imdr = $this->getServiceItemMaterialDocumentRelation()->getListByItemId($m_item_prog->getItem()
                ->getId());
            $ar_imdr = array();
