...
class MessageUser extends AbstractMapper
{
    ...
    public function getList($me, $message = null, $conversation = null, $tag = 'INBOX', $type = null, $filter = null)
    {
        ...
        $select->columns(array('id','user_id','from_id','read_date','conversation_id','created_date'))
            ->join(array('message_user_message' => 'message'), 'message_user_message.id=message_user.message_id', array('id','is_draft','type','text','token','message$created_date' => new Expression("DATE_FORMAT(message_user_message.created_date, '%Y-%m-%dT%TZ') ")))
