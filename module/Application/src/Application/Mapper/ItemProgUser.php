...
class ItemProgUser extends AbstractMapper
{
    ...
    public function getStartedConference($user)
    {
        ...
        $select->columns(array('item_prog_user$started_date' => new Expression("DATE_FORMAT(item_prog_user.started_date, '%Y-%m-%dT%TZ') ")))
            ...
            ->join('questionnaire', 'questionnaire.item_id=item.id', array('id', 'created_date'), $select::JOIN_LEFT)
            ->join('questionnaire_user', 'questionnaire.id=questionnaire_user.questionnaire_id AND questionnaire_user.user_id=item_prog_user.user_id', array(), $select::JOIN_LEFT)
