...
class Item extends AbstractMapper
{
    public function getListGrade($user, $programs, $courses, $type, $notgraded, $newMessage, $filter)
    {
        ...
        $select->columns(array('id', 'title', 'item$new_message' => $select_new_message))
               ...
               ->join(array('item_item_prog_item_assignment' => 'item_assignment'), 'item_item_prog_item_assignment.item_prog_id=item_prog.id', array('id', 'item_item_prog_item_assignment$submit_date' => new Expression('DATE_FORMAT(submit_date, "%Y-%m-%dT%TZ")')), $select::JOIN_LEFT)
               ->join('item_assignment_user', 'item_assignment_user.item_assignment_id=item_item_prog_item_assignment.id AND item_assignment_user.user_id=item_prog_user.user_id', array(), $select::JOIN_LEFT)
               ...
               ->where(array('( item_item_prog_item_assignment.id IS NULL OR item_assignment_user.user_id IS NOT NULL)'))
               ->where(array('( item_item_prog_item_assignment.id IS NOT NULL OR item_prog.due_date IS NOT NULL)'))
        ...
        if($notgraded === true || $newMessage === true){
            if ($newMessage === true) {
                $select->join('item_assignment_comment', 'item_assignment_comment.item_assignment_id=item_item_prog_item_assignment.id', array(), $select::JOIN_LEFT)
