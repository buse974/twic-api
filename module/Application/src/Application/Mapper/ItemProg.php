...
class ItemProg extends AbstractMapper
{
     ...
     */
    public function getList($user, $item = null, $start = null, $end = null, $course = null)
    {
        $select = $this->tableGateway->getSql()->select();
        
        $select->columns(array('id','item_prog$start_date' => new Expression("DATE_FORMAT(start_date, '%Y-%m-%dT%TZ') "),'item_prog$due_date' => new Expression("DATE_FORMAT(due_date, '%Y-%m-%dT%TZ') ")));
        ...
        if (in_array(\Application\Model\Role::ROLE_INSTRUCTOR_STR, $user['roles'])) {
            $select->columns(array('id','item_prog$editable' => new Expression('1'),'item_prog$start_date' => new Expression("DATE_FORMAT(start_date, '%Y-%m-%dT%TZ') "),'item_prog$due_date' => new Expression("DATE_FORMAT(due_date, '%Y-%m-%dT%TZ') ")));
